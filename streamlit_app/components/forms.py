...
def form_nouveau_sinistre(
    contrat_id: int, key_prefix: str = "new_sinistre"
) -> Optional[dict]:
    """Formulaire déclaration sinistre pour un contrat actif."""
    with st.form(key=f"{key_prefix}_form"):
        st.write(f"**Contrat ID** : {contrat_id}")
        type_sinistre = st.selectbox(
            "Type de sinistre",
            [
                "hospitalisation",
                "consultation",
        ...
        montant = st.number_input(
            "Montant (USD)", min_value=1.0, value=500.0, step=50.0
        )
        date_sinistre = st.date_input("Date du sinistre", value=date.today())
        description = st.text_area("Description (optionnel)", height=80)

        submitted = st.form_submit_button(
            "Déclarer le sinistre", use_container_width=True, type="primary"
        )
